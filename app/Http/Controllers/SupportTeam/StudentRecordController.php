...
use App\Exports\StudentTemplateData;
use App\Helpers\Qs;
use App\Helpers\Mk;
...
class StudentRecordController extends Controller
{
   ...
   public function __construct(LocationRepo $loc, MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
    ...
    public function upload_student($class_id, $section_id)
    {
        if(request()->post()) {
            $importStudent = new ImportStudentData($class_id, $section_id);
            Excel::import($importStudent, request()->file('student_file'));
            return back()->with('flash_success', __('msg.bulk_upload_success'));
        }

        return back()->with('flash_success', __('msg.invalid_request'));
    }

    public function download_template($class_id, $section_id) {
        $class =  $this->my_class->find($class_id);
        $section =  $this->my_class->findSection($section_id);

        return Excel::download(new StudentTemplateData($class_id, $section_id), 'student_template-'.$class->name.'.'.$section->name.'.xlsx');
    }
}
