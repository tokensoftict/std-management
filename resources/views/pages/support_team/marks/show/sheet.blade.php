<table class="table table-bordered table-responsive text-center">
    ...
    @foreach($subjects as $sub)
        <tr>
            ...
            @foreach($marks->where('subject_id', $sub->id)->where('exam_id', $ex->id) as $mk)
                ...
                <td>
                    @if($ex->term === 1) {{ ($mk->class_av1) }}
                    @elseif ($ex->term === 2) {{ ($mk->class_av2) }}
                    @elseif ($ex->term === 3) {{ ($mk->class_av3) }}
                    @else {{ '-' }}
                    @endif
                </td>
                <td>
                    @if($ex->term === 1) {{ ($mk->h_score1) }}
                    @elseif ($ex->term === 2) {{ ($mk->h_score2) }}
                    @elseif ($ex->term === 3) {{ ($mk->h_score3) }}
                    @else {{ '-' }}
                    @endif
                </td>
    ...
    <tr>
        ...
        <td colspan="6"><strong>CLASS AVERAGE: </strong> {{ $exr->class_ave }}</td>
    </tr>
    </tbody>
</table>
