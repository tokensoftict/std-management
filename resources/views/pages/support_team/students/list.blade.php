            ...
            <div class="tab-content">
                ...
                @foreach($sections as $se)
                    <div class="tab-pane fade" id="s{{$se->id}}">
                        <a href="#" data-toggle="modal" data-target="#uploadStudent{{$se->id}}" class="btn btn-primary btn-lg">Upload Student</a>
                        <div class="modal fade" id="uploadStudent{{$se->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route("students.upload", [$se->my_class_id, $se->id]) }}" enctype="multipart/form-data" method="post">
                                        {{ csrf_field() }}
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="uploadStudent{{$se->id}}">Upload Student</h5>
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-sm-12">
                                                    <input type="hidden" value="{{ $se->my_class_id }}" name="class_id">
                                                    <input type="hidden" value="{{ $se->id }}" name="section_id">
                                                    <div class="form-group">
                                                        <label>Session: <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control" disabled value="{{ Qs::getSetting('current_session') }}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Class: <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control" disabled value="{{ $se->my_class->name }}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Section: <span class="text-danger">*</span></label>
                                                        <input type="text" class="form-control" disabled value="{{ $se->name }}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>File: <span class="text-danger">*</span></label>
                                                        <input type="file" class="form-control" name="student_file">
                                                    </div>
                                                    <a target="_blank" href="{{ route('students.download_template',[$se->my_class_id, $se->id ]) }}">Download Student Template</a>
                                                </div>
                                            </div>
                                        </div>
